...
var source = (() => {
  ...
  var MangaPlusExtension = class {
    ...
    async getFeaturedTitles() {
      ...
      const results = result.success?.featuredTitlesViewV2?.contents?.find((x) => x.titleList && x.titleList.listName == "WEEKLY SHONEN JUMP")?.titleList.featuredTitles.filter(
    ...
    async getSearchResults(query, metadata) {
      ...
      const results = result.success?.allTitlesViewV2?.AllTitlesGroup.flatMap(
        (group) => group.titles
      ).filter((title2) => languages.includes(title2.language ?? "ENGLISH" /* ENGLISH */)).filter(
